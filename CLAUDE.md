...
- `t0`-`t3` — quick diagnostic shortcuts on ucatsb (`~/bin/t*` symlinked to these repo files in `~/code`): `t0` NTP sync server + `timedatectl status`, `t1` ping the aircraft gateway (`10.11.96.1`), `t2` ping the building 994 gateway (`10.16.101.1`), `t3` watch UDP telemetry `sendto()` calls from a running `instrument.py` for a few seconds (absorbs what was `udp-check.sh`)
